...
from training.layers import GenInput
...
class SynthesisBlock(torch.nn.Module):
    def __init__(
        ...
        **layer_kwargs,                                 # Arguments for SynthesisLayer.
        ...
        if in_channels == 0:
            self.input = GenInput(cfg, out_channels, motion_v_dim=motion_v_dim)
            self.num_conv += 1
            convs.append(SynthesisLayer(self.input.total_dim, out_channels, w_dim=w_dim, resolution=resolution,
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(
        ...
        base_mult: int = 2,         # Start resolution (SG2: 2, SG3: 4, SG-T: 3).
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlock(in_channels, out_channels, cfg=cfg, w_dim=w_dim, resolution=res,
                                   img_channels=img_channels, is_last=is_last, use_fp16=use_fp16,
                                   num_res_blocks=num_res_blocks, motion_v_dim=self.motion_v_dim, **block_kwargs)
            self.num_ws += block.num_conv
            if is_last:
                self.num_ws += block.num_torgb
            setattr(self, f'b{res}', block)

    def forward(self, ws: torch.Tensor, t=None, c=None, motion_z=None, motion_v=None, **block_kwargs) -> torch.Tensor:
        block_ws = []
...
class Generator(torch.nn.Module):
    def __init__(
        ...
        self.conditional = (c_dim > 0)
        self.img_resolution = img_resolution
        self.img_channels = img_channels

        self.mapping = MappingNetwork(z_dim=self.z_dim, conditional=self.conditional)
        self.synthesis = SynthesisNetwork(w_dim=self.mapping.w_dim, img_resolution=img_resolution,
                                          img_channels=img_channels, cfg=cfg, **synthesis_kwargs)
    ...
    def forward(
        ...
        img = self.synthesis(ws, c=c, t=t, **synthesis_kwargs)
        return img
